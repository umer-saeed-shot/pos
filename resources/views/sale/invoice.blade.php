...
<div style="max-width:400px;margin:0 auto">
    ...
    <div class="hidden-print">
        <table>
            <tr>
                <td><a href="{{$url}}" class="btn btn-info"><i class="fa fa-arrow-left"></i> Close</a> </td>
    ...
    <div id="receipt-data">
        <div class="centered">
            @if($general_setting->site_logo)
                <img src="{{url('public/img/logo/', $logo)}}" height="60" width="60" style="margin:10px 0;">
            @endif

            <h2>{{$lims_biller_data->company_name}}</h2>

            <p>Address: {{$lims_warehouse_data->address}}

                <br>Phone Number: {{$lims_warehouse_data->phone}}<br>
            </p>


        </div>
        <p>Date: {{$lims_sale_data->updated_at->format('Y-m-d')}} - {{$lims_sale_data->updated_at->format('h:i:s a')}} <br>
            Reference: {{$lims_sale_data->reference_no}}<br>
        ...
        <table>
            <tbody>
                ...
                <tr style="background-color:#ddd;">
                    ...
                    <td style="padding: 5px;width:40%">{{trans('file.Amount')}} ({{ auth()->user()->currency }}): {{number_format((float)$payment_data->amount, 2, '.', '')}}</td>
                    <td style="padding: 5px;width:30%">{{trans('file.Change')}} ({{ auth()->user()->currency }}): {{number_format((float)$payment_data->change, 2, '.', '')}}</td>
                </tr>
                @endforeach

                <tr><td><h3>Sale Note</h3></td></tr>
                <tr style="background-color:#ddd;"><td colspan="3" style="padding: 5px">{{ $lims_sale_data->sale_note }}</td></tr>
                <tr><td class="centered greeting" colspan="3">{{'Thank you for visiting! Please consider leaving a review on Google Maps, and clicking the "Check-in" button on YELP!'}}</td></tr>
                <tr>
