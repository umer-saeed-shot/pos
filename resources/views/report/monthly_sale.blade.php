					...
					<table class="table table-bordered" style="border-top: 1px solid #dee2e6; border-bottom: 1px solid #dee2e6;">
					    ...
					    <tbody>
						    ...
						    <tr>
						        ...
						        <td>
						        	@if($discount > 0)
							      	<strong>{{trans("file.Product Discount")}} ({{ auth()->user()->currency }})</strong><br>
							      	<span>{{$discount}}</span><br><br>
							      	@endif
							      	@if($order_discount[$key] > 0)
							      	<strong>{{trans("file.Order Discount")}} ({{ auth()->user()->currency }})</strong><br>
							      	<span>{{$order_discount[$key]}}</span><br><br>
							      	@endif
							      	@if($total_tax[$key] > 0)
							      	<strong>{{trans("file.Product Tax")}} ({{ auth()->user()->currency }})</strong><br>
							      	<span>{{$total_tax[$key]}}</span><br><br>
							      	@endif
							      	@if($order_tax[$key] > 0)
							      	<strong>{{trans("file.Order Tax")}} ({{ auth()->user()->currency }})</strong><br>
							      	<span>{{$order_tax[$key]}}</span><br><br>
							      	@endif
							      	@if($shipping_cost[$key] > 0)
							      	<strong>{{trans("file.Shipping Cost")}} ({{ auth()->user()->currency }})</strong><br>
							      	<span>{{$shipping_cost[$key]}}</span><br><br>
							      	@endif
							      	@if($total[$key] > 0)
							      	<strong>{{trans("file.grand total")}} ({{ auth()->user()->currency }})</strong><br>
