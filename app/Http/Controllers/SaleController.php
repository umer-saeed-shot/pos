...
class SaleController extends Controller
{
    ...
    {
	//$recent_draft = Sale::where('payment_status', 2)->orderBy('id', 'desc')->take(10)->get();
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('sales-index')) {
            $permissions = Role::findByName($role->name)->permissions;
            foreach ($permissions as $permission)
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_sale_all = Sale::orderBy('id', 'desc')->where('user_id', Auth::id())->where('payment_status', 2)->get();
            else
                $lims_sale_all = Sale::orderBy('id', 'desc')->where('payment_status', 2)->get();

            $lims_gift_card_list = GiftCard::where("is_active", true)->get();
            $lims_pos_setting_data = PosSetting::latest()->first();
            $lims_account_list = Account::where('is_active', true)->get();

            return view('sale.indexHold',compact('lims_sale_all', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_account_list', 'all_permission'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
        ...
        try {
            ...
            if (!empty($sales)) {
                foreach ($sales as $key=>$sale) {
                    ...
                    $nestedData['key'] = $key;
                    $nestedData['date'] = date("Y/m/d", strtotime($sale->created_at->toDateString())) . '<br>' . $sale->created_at->format('h:i:s a');
                    $nestedData['reference_no'] = $sale->reference_no;
                    $nestedData['biller'] = $sale->biller->name;
                    $nestedData['store'] = $sale->warehouse->name;
                    ...
                    $nestedData['cash_payment'] = number_format($sale->cash_payment, 2);
                    $nestedData['card_payment'] = number_format($sale->card_payment, 2);
                    $nestedData['cheque_payment'] = number_format($sale->cheque_payment, 2);
                    $nestedData['e_transfer_payment'] = number_format($sale->e_transfer_payment, 2);
    ...
    public function unholdPosSale($id){
    	$sale = Sale::where('id',$id)->first();

        Session::put('draft_id',$id);
        Session::put('customer_id',$sale['customer_id']);
        return redirect()->route('sale.pos');
    }

    public function saleDataHold(Request $request)
    {
            ...
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                ...
                                $nestedData['options'] .= '<li>
                            <a href="'.route('sale.unholdpos', $sale->id).'" class="btn btn-link"><i class="dripicons-document-edit"></i>Unhold</a>
                            </li>';
                if(in_array("sales-edit", $request['all_permission'])){
                    if($sale->sale_status != 3)
                        $nestedData['options'] .= '<li>
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            if($role->hasPermissionTo('tax_all')){
                $lims_tax_list = Tax::where('is_active', true)->get();
            }else{
                $lims_tax_list = Tax::where('id', auth()->user()->tax_id)->get();
            }
            $lims_pos_setting_data = PosSetting::latest()->first();
    ...
    public function store(Request $request)
    {
		...
		if ( $data['sale_status'] == 3)
		{
            ...
            if($data['paying_amount'] < $data['paid_amount'] ){
                $data['payment_status'] = 2;
            }else{
        ...
        if($data['misc_ids'] !== null){
            $ids = explode(',',$data['misc_ids']);
            $costs = explode(',',$data['misc_costs']);
            $taxes = explode(',',$data['misc_taxes']);
            $discounts = explode(',',$data['misc_discounts']);

            for ($i=0; $i < count($ids) ; $i++) {
                $misc_product = Product::where('id',$ids[$i])->first();

                $lims_product_warehouse_data = new Product_Warehouse();
                $lims_product_warehouse_data->product_id = $misc_product['id'];
                $lims_product_warehouse_data->variant_id = null;
                $lims_product_warehouse_data->price = $misc_product['price'];
                $lims_product_warehouse_data->warehouse_id = auth()->user()->warehouse_id;
                $lims_product_warehouse_data->qty = $misc_product['qty'];
                ...
                if($taxes[$i] == 0){
                    ...
                    $tax_rate = $taxes[$i];
                    $total_tax = floatval($misc_product['price'])*$misc_product['qty']*(floatval($taxes[$i])/100);
                ...
                $purchase_data['warehouse_id'] = auth()->user()->warehouse_id;
                $purchase_data['reference_no'] = 'special-order-' . date("Ymd") . '-'. date("his");
                $purchase_data['item'] = 1;
                $purchase_data['total_qty'] = $misc_product['qty'];
                $purchase_data['total_discount'] = $discounts[$i];
                $purchase_data['total_tax'] = $total_tax;
                $purchase_data['total_cost'] = floatval($costs[$i])*$misc_product['qty'];
                $purchase_data['order_tax_rate'] = $tax_rate;
                $purchase_data['order_tax'] = $total_tax;
                $purchase_data['grand_total'] = floatval($costs[$i])*$misc_product['qty'];
                $purchase_data['paid_amount'] = floatval($costs[$i])*$misc_product['qty'];
                $purchase_data['status'] = 1;
                $purchase_data['payment_status'] = 2;
                $purchase_data['note'] = "This is special order having misc item";
                ...
                $product_purchase['product_id'] = $misc_product['id'];
                $product_purchase['qty'] = $misc_product['qty'];
                $product_purchase['recieved'] = $misc_product['qty'];
                $product_purchase['purchase_unit_id'] = 1;
                $product_purchase['net_unit_cost'] = $costs[$i];
                $product_purchase['discount'] = $discounts[$i];
                $product_purchase['tax_rate'] = $tax_rate;
                $product_purchase['tax'] = $total_tax;
                $product_purchase['total'] = floatval($costs[$i])*$misc_product['qty'];
                ProductPurchase::create($product_purchase);
            }

        }
        $paid_amount = 0;
        if(isset($data['paying_amount_array'])){
            for($i=0;$i < count($data['paying_amount_array']);$i++){
                if($data['paid_by_id_array'][$i] == 1){
                    $data['cash_payment'] = $data['paying_amount_array'][$i];
                }elseif($data['paid_by_id_array'][$i] == 3){
                    $data['card_payment'] = $data['paying_amount_array'][$i];
                }elseif($data['paid_by_id_array'][$i] == 4){
                    $data['cheque_payment'] = $data['paying_amount_array'][$i];
                }elseif($data['paid_by_id_array'][$i] == 7){
                    $data['e_transfer_payment'] = $data['paying_amount_array'][$i];
                }
                $paid_amount += $data['paying_amount_array'][$i];

            }
            $data['paid_amount'] = $paid_amount;
            if($paid_amount < $data['grand_total']){
                $data['payment_status'] = 2;
            }else{
                $data['payment_status'] = 4;
            }
            ...
            if($data['paid_by_id'] == 1)
                $data['cash_payment'] = $data['paying_amount'];
            elseif ($data['paid_by_id'] == 3)
                $data['card_payment'] = $data['paying_amount'];
            elseif ($data['paid_by_id'] == 4)
                $data['cheque_payment'] = $data['paying_amount'];
            elseif ($data['paid_by_id'] == 7)
                $data['e_transfer_payment'] = $data['paying_amount'];
        }
        ...
        $mail_data['order_tax'] = $lims_sale_data->total_tax;
        ...
        $mail_data['cash_amount'] = $lims_sale_data->cash_payment;
        $mail_data['card_amount'] = $lims_sale_data->card_payment;
        $mail_data['cheque_amount'] = $lims_sale_data->cheque_payment;
        $mail_data['e_transfer_amount'] = $lims_sale_data->e_transfer_payment;
        $mail_data['sale_note'] = $lims_sale_data->sale_note;
//echo '<pre>'; print_r($mail_data); exit;
        ...
        foreach ($product_id as $i => $id) {
            ...
            $product_qty = $product_data[0]->qty;
            $product_alert_qty = $product_data[0]->alert_quantity;

            if($product_qty <= $product_alert_qty ){

                    $users = User::where('id',$product_data[0]->user_id)->where('is_active',true)->get();
                    $pro = ['products' => $product_data];
                    if($users && $product_data[0]->alert_quantity !== 0){
                        foreach ($users as $user) {
                            Mail::send( 'mail.stock_alert', $pro, function( $message ) use ($user,$product_data)
                            {
                                $message->to( $user->email )->subject( "Stock Alert - ".$product_data[0]->name." (".$product_data[0]->code.") " );
                            });
                        }
                    }
            ...
            $product_sale['total'] = $mail_data['total'][$i] = $net_unit_price[$i];
        ...
        if($mail_data['email'] && $isHold == false) {
            try {
        ...
        if($mail_data['email'] && $isHold == true) {
            try {
                Mail::send( 'mail.saleheld_details', $mail_data, function( $message ) use ($mail_data)
                {
                    $message->to( $mail_data['email'] )->subject( 'Sale Held Details' );
                });
            }
            catch(\Exception $e){
        ...
        if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            if(!isset($data['paid_by_id_array'])){
                if($data['paid_by_id'] == 1)
                    $paying_method = 'Cash';
                ...
                $lims_payment_data->payment_note = 'None';
                ...
                //dd($paying_method);
                if($paying_method == 'Card'){

                        $data['customer_id'] = $lims_sale_data->customer_id;
                        $data['charge_id'] = $data['cardnumber'];
                        $data['payment_id'] = $lims_payment_data->id;
                        $data['customer_stripe_id'] = $lims_sale_data->customer_id;
                ...
                for($i=0;$i < count($data['paying_amount_array']);$i++){
                    if($data['paid_by_id_array'][$i] == 1)
                        $paying_method = 'Cash';
                    elseif ($data['paid_by_id_array'][$i] == 2){
                        $paying_method = 'Gift Card';
                    }
                    elseif ($data['paid_by_id_array'][$i] == 3)
                        $paying_method = 'Card';
                    elseif ($data['paid_by_id_array'][$i] == 4)
                        $paying_method = 'Cheque';
                    elseif ($data['paid_by_id_array'][$i] == 5)
                        $paying_method = 'Paypal';
                    elseif ($data['paid_by_id_array'][$i] == 7)
                        $paying_method = 'E-Transfer';
                    else
                    ...
                    $data['payment_reference'] = 'spr-'.date("Ymd").'-'.date("his");
                    $lims_payment_data->payment_reference = $data['payment_reference'];
                    $lims_payment_data->amount = $data['paying_amount_array'][$i];
                    $lims_payment_data->change = $data['paying_amount_array'][$i] - $change;
                    $lims_payment_data->paying_method = $paying_method;
                    $lims_payment_data->payment_note = $data['payment_note_array'][$i];
                    //dd($lims_payment_data);
                    $lims_payment_data->save();
                    ...
                    elseif ($paying_method == 'Gift Card') {
                        $lims_gift_card_data = GiftCard::find($data['gift_card_id']);
                        $lims_gift_card_data->expense += $data['paying_amount_array'][$i];
                        $lims_gift_card_data->save();
                        PaymentWithGiftCard::create($data);
                    }
                    elseif ($paying_method == 'Cheque') {
                    ...
                    elseif($paying_method == 'Deposit'){
                        $lims_customer_data = Customer::find($data['customer_id']);
                        $lims_customer_data->expense += $data['paying_amount_array'][$i];
                        $lims_customer_data->save();
                    }
                    $change = $change - $data['paying_amount_array'][$i];
                }
    ...
    public function getProduct($id)
    {
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse)
        {
            ...
            $phouse = Warehouse::find($product_warehouse->warehouse_id)->get('name');
            $productwarehouse[] = $phouse[0]->name;
...
/*
 		...
 		*/
/////////////////////////////////////////////////////////////
        $product_data = [$product_code, $product_name, $product_qty, $product_type, $product_id, $product_list, $qty_list, $product_price,$productwarehouse];
        return $product_data;
    }

    public function posSale($draft_id = null)
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            if(empty($all_permission))
                $all_permission[] = 'dummy text';
            $lims_customer_list = Customer::where('is_active', true)->orderBy('id', 'desc')->get();
    ...
    public function getHeldProducts($id){
        $sale_id = $id;
        // return $id;
        $product=[];
        try{
            $product_sale = Product_Sale::where('sale_id',$sale_id)
                ->where('products.is_active',true)
                ->join('products','product_sales.product_id','products.id')
                // ->join('taxes','products.tax_id','taxes.id')
                ->get(['products.name as name','products.code as code','product_sales.qty as qty','product_sales.net_unit_price as unit_price','product_sales.tax_rate as tax_rate','products.tax_id as tax_id','products.type as type','products.unit_id as unit_id','products.sale_unit_id as sale_unit_id','products.tax_method as tax_method','products.tax_type as tax_type','products.cost as cost','products.id as product_id']);
                //
            ...
            foreach($payments as $payment){
                if($payment->paying_method == "Cheque"){
                    $payment_with_cheque = PaymentWithCheque::where('payment_id',$payment->id)->first();
                    $sale['cheque_no'] = $payment_with_cheque['cheque_no'];
                    $payment_with_cheque->delete();
                }elseif($payment->paying_method == "Card"){
                    $payment_with_card = PaymentWithCreditCard::where('payment_id',$payment->id)->delete();
                }
            }
        ...
        foreach($product_sale as $pro){
            $temp_product = [];
            $temp_product[] = $pro->name; //1
            $temp_product[] = $pro->code; //2
            $temp_product[] = $pro->unit_price; //3


            if($pro->tax_id) {
                $temp_product[] = $pro->tax_rate; //4
                $tax_data = Tax::find($pro->tax_id);
                $temp_product[] = $tax_data->name; //5

            }
            else{
                $temp_product[] = 0; //4
                $temp_product[] = 'No Tax'; //5
            }
            $temp_product[] = $pro->tax_method; //6

            if($pro->type == 'standard'){
                $units = Unit::where("base_unit", $pro->unit_id)
                        ->orWhere('id', $pro->unit_id)
                        ->get();
                $unit_name = array();
                $unit_operator = array();
                $unit_operation_value = array();
                foreach ($units as $unit) {
                    if($pro->sale_unit_id == $unit->id) {
                ...
                $temp_product[] = implode(",",$unit_name) . ',';
                $temp_product[] = implode(",",$unit_operator) . ',';
                $temp_product[] = implode(",",$unit_operation_value) . ',';
            }
            else{
                $temp_product[] = 'n/a'. ','; //7
                $temp_product[] = 'n/a'. ','; //8
            ...
            $temp_product[] = $pro->product_id; //10
            ...
            $temp_product[] = $product_warehouse->qty; //11
            $temp_product[] = $pro->qty; // 12
            $temp_product[] = $pro->tax_type; // 13 - index[12]
            $temp_product[] = $pro->cost; // 14 - index[13]




            array_push($product,$temp_product);

        }

        Sale::where('id',$id)->delete();
        Product_Sale::where('sale_id',$sale_id)->delete();

        return ['products'=>$product,'sale'=>$sale];
    ...
    public function addMiscItem(Request $request)
    {
        ...
        $misc_cat = Category::where('name',"Miscellaneous")->first();
        $misc_type = Type::where('name',"Miscellaneous")->first();
        if($misc_cat !== null){

            $cat_id = $misc_cat['id'];
        }else{
            $misc_cat = Category::create(['name'=>"Miscellaneous","is_active"=>1]);
            $cat_id = $misc_cat->id;
        }

        if($misc_type !== null){

            $type_id = $misc_type['id'];
        }else{
            $misc_type = Type::create(['name'=>"Miscellaneous","is_active"=>1]);
            $type_id = $misc_type->id;
        }
        ...
        $add_misc_product = new Product();
        $add_misc_product->name = $request->name;
        $add_misc_product->code = $code;
        $add_misc_product->type = $type_id;
        $add_misc_product->image = "zummXD2dvAtI.png";
        $add_misc_product->category_id = $request->category_id;
        $add_misc_product->cost = $request->cost;
        $add_misc_product->price = $request->price;
        $add_misc_product->qty = $request->qty;
        $add_misc_product->alert_quantity = 0;
        $add_misc_product->tax_method = 1;
        $add_misc_product->tax_type = $request->tax_type;
        $add_misc_product->featured = 1;
        $add_misc_product->is_active = 1;
        $add_misc_product->save();

        $product_id = $add_misc_product->id;

        // $lims_product_warehouse_data = new Product_Warehouse();
        // $lims_product_warehouse_data->product_id = $product_id;
        // $lims_product_warehouse_data->variant_id = null;
        // $lims_product_warehouse_data->price = $request->price;
        // $lims_product_warehouse_data->warehouse_id = auth()->user()->warehouse_id;
        // $lims_product_warehouse_data->qty = $request->qty;
        // $lims_product_warehouse_data->save();



        if($request->tax == 0){
            $tax = Tax::where('id',auth()->user()->tax_id)->first();
            $tax_rate = $tax['rate'];
            $total_tax = floatval($request->total_price)*(floatval($tax_rate)/100);
            // $product_tax =  floatval($request->price)*(floatval($request->tax)/100);
            $tax_name = $tax['name'];
        ...
        try {
            ...
            // $data['total_cost'] = floatval($request->cost)*$request->qty;
            // $data['order_tax_rate'] = $tax_rate;
            // $data['order_tax'] = $total_tax;
            // $data['grand_total'] = floatval($request->cost)*$request->qty;
            // $data['paid_amount'] = floatval($request->cost)*$request->qty;
            // $data['status'] = 1;
            // $data['payment_status'] = 2;
            ...
            // $product_purchase['purchase_unit_id'] = 1;
            // $product_purchase['net_unit_cost'] = $request->cost;
            // $product_purchase['discount'] = $request->discount;
            // $product_purchase['tax_rate'] = $tax_rate;
            // $product_purchase['tax'] = $total_tax;
            // $product_purchase['total'] = floatval($request->cost)*$request->qty;
            // ProductPurchase::create($product_purchase);
            ...
            $temp_product[] = $pro["name"]; //1
            $temp_product[] = $pro["code"]; //2
            $temp_product[] = $request->price; //3
            $temp_product[] = $tax_rate; //4
            $temp_product[] = $tax_name; //5
            $temp_product[] = $pro["tax_method"]; //6
            $temp_product[] = 'n/a'. ','; //7
            $temp_product[] = 'n/a'. ','; //8
            $temp_product[] = 'n/a'. ','; //9

            $temp_product[] = $product_id; //10
            $temp_product[] = $request->qty; //11
            $temp_product[] = $request->qty; //12
            $temp_product[] = $pro['tax_type']; //13
            $temp_product[] = $pro['cost']; //14
            return $temp_product;
        }catch(\Exception $ex){
            return $ex;
        }
    ...
    public function deleteMiscProducts(Request $request){
        $ids = $request->ids;
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product[] = $lims_product_data->name; // 1
        if($lims_product_data->is_variant){
            $product[] = $lims_product_data->item_code; // 2
            $lims_product_data->price += $lims_product_data->additional_price;
        }
        else
            $product[] = $lims_product_data->code; // 2

        if($lims_product_data->promotion && $todayDate <= $lims_product_data->last_date){
            $product[] = $lims_product_data->promotion_price; //3
        }
        else
            $product[] = $lims_product_data->price; //3

        if($lims_product_data->tax_id) {
            $lims_tax_data = Tax::find($lims_product_data->tax_id);
            $product[] = $lims_tax_data->rate; // 4
            $product[] = $lims_tax_data->name; //5
        }
        else{
            $product[] = 0; // 4
            $product[] = 'No Tax'; // 5
        }
        $product[] = $lims_product_data->tax_method; //6
        ...
        $product[] = $product_variant_id; // 11
        $product[] = $lims_product_data->promotion; // 12
        //echo $userdata->warehouse_id; exit;
        ...
        $product[] = $product_warehouse->qty; //13
        }
        else
        {
         $product[] = 0; //13
        }
        $product[] = $lims_product_data->tax_type; // 14 - index[13]
        $product[] = $lims_product_data->cost; // 15 - index[14]
        //dd($product);
        return $product;
    ...
    public function genInvoice($id)
    {
        ...
        */
        ...
        {
        $lims_payment_data[]->put('change', $lims_sale_data['total_price']-$lims_sale_data['paid_amount']);
        $lims_payment_data[]->put('paying_method','');
        $lims_payment_data[]->put('amount','');
        }*/
        $numberToWords = new NumberToWords();
        if(\App::getLocale() == 'ar' || \App::getLocale() == 'hi' || \App::getLocale() == 'vi' || \App::getLocale() == 'en-gb')
            $numberTransformer = $numberToWords->getNumberTransformer('en');
...
//echo $paying_method; exit;
        ...
        elseif($paying_method == 'Card'){
            if($data['extcard'] == true)
            {
                /*
                $data['customer_id'] = $lims_sale_data->customer_id;
                $data['charge_id'] = $data['cardnumber'];
    ...
    public function addPaymentt(Request $request)
    {
        $data = $request->all();
		//echo '<pre>'; print_r($data); exit;
        if(!$data['amount'])
...
//echo $paying_method; exit;
    ...
    public function addPaymenttH(Request $request)
    {
        $data = $request->all();
		//dd($data);
...
//echo $paying_method; exit;
        ...
        if($cash_register_data)
            $lims_payment_data->cash_register_id = $cash_register_data->id;
        $lims_payment_data->account_id = $data['paid_by_id'];
        $data['payment_reference'] = 'spr-' . date("Ymd") . '-'. date("his");
        $lims_payment_data->payment_reference = $data['payment_reference'];
        $lims_payment_data->amount = $data['amount'];
        $lims_payment_data->change = $data['paying_amount'] - $data['amount'];
    ...
    public function updatePayment(Request $request)
    {
        ...
        if($data['edit_paid_by_id'] == 7)
            $lims_payment_data->paying_method = 'E-Transfer';
        ...
        elseif ($data['edit_paid_by_id'] == 3){
            $lims_pos_setting_data = PosSetting::latest()->first();
            Stripe::setApiKey($lims_pos_setting_data->stripe_secret_key);
            if($lims_payment_data->paying_method == 'Card'){
    ...
    public function deleteBySelection(Request $request)
    {
        ...
        //echo '<pre>'; print_r($sale_id); exit;
        foreach ($sale_id as $id) {
            $lims_sale_data = Sale::find($id);
            //echo '<pre>'; print_r($lims_sale_data); exit;
            $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
            ...
            //echo '<pre>'; print_r($lims_product_sale_data); exit;
            ...
            $err = "";
            $lims_payment_data = Payment::where('sale_id', $id)->get();
            //echo 'id:'.$id; print_r($lims_payment_data);
