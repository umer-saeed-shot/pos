...
class SettingController extends Controller
{
    ...
    public function emailMessages(){
        $email_messages = EmailMessage::where('shop_owner_id',auth()->user()->id)->get();

        return view('setting.email_messages')->with('table',$email_messages);
    }

    public function storeEmailMessages(Request $request){
        $message = EmailMessage::where('shop_owner_id',auth()->user()->id)->where('type',$request->type)->first();

        if($message == null){
            $new_message = new EmailMessage();
            $new_message->type = $request->type;
            $new_message->message = $request->message;
            $new_message->shop_owner_id = auth()->user()->id;
            $new_message->save();

            return back()->with('message','Email Message Added!');
        }else{
            EmailMessage::where('id',$message['id'])->update(['message'=>$request->message]);
            return back()->with('message','Email Message Edited!');
        }
    }

    public function deleteEmailMessage($id)
    {
        EmailMessage::where('id',$id)->delete();
        return back()->with('message','Email Message Deleted Successfully!');
    }
    ...
    public function mailSettingStore(Request $request)
    {
        ...
        */
		...
		$newContents = str_replace('MAIL_USERNAME='.env('MAIL_USERNAME'), 'MAIL_USERNAME='.$data['mail_address'], $newContents);
		$newContents = str_replace('MAIL_PASSWORD='.env('MAIL_PASSWORD'), 'MAIL_PASSWORD='.$data['password'], $newContents);
		$newContents = str_replace('MAIL_ENCRYPTION='.env('MAIL_ENCRYPTION'), 'MAIL_ENCRYPTION='.$data['encryption'], $newContents);
		$newContents = str_replace('MAIL_FROM_NAME='.env('MAIL_FROM_NAME'), 'MAIL_FROM_NAME='.str_replace(' ', '',$data['mail_name']), $newContents);
