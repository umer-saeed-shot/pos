...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        if($role->hasPermissionTo('users-index')){
            ...
            $user=null;
            $i=0;
            foreach ($lims_user_list as $lims_user_listt) {
                //@$lims_user_list[$i]->{'wname'}="";
                if($lims_user_listt->warehouse_id != null)
                {
                    $whouse=Warehouse::select('name')->where('id',$lims_user_listt->warehouse_id)->get();
                    //echo $whouse[0]->name;
                    if($whouse)
                    $lims_user_listt->remember_token=$whouse[0]->name;
                    else
                    $lims_user_listt->remember_token="";
                }
                //$i++;
            }
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('users-add')){
            ...
            return view('user.create', compact('lims_role_list', 'lims_biller_list',"lims_currency_list", 'lims_warehouse_list', 'lims_customer_group_list','lims_tax_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
