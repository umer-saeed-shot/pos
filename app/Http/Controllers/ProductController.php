...
class ProductController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('products-index')){
            ...
            if(empty($all_permission))
                ...
                //echo '<pre>'; print_r($all_permission); exit;
    ...
    public function productData(Request $request)
    {
        ...
        if(!empty($products))
        {
            ...
            {
                $average_cost = 0;
                $product_purchases = ProductPurchase::where('product_id',$product->id)->get();
                if(!empty($product_purchases)){
                    $quantity = 0;
                    $cost = 0;
                    foreach($product_purchases as $pp){
                        $quantity += $pp->qty;
                        $cost += $pp->net_unit_cost * $pp->qty;
                    }
                    if($quantity !== 0){

                        $average_cost = number_format((float)$cost / $quantity, 2, '.', '');;
                    }
                ...
                $nestedData['last_cost'] = $product->cost;
                $nestedData['average_cost'] = $average_cost;
                if($product->brand_id)
                ...
                else
                {
                    ...
                    $nestedData['qty'] = $lims_product_warehouse_data->qty;
                    else
                    $nestedData['qty'] = 0;
                }
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('products-add')){
            $lims_type_list = Type::where('is_active',true)->get();
            ...
            return view('product.create',compact('lims_product_list', 'lims_brand_list',"lims_type_list", 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_warehouse_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lims_warehouse_list = Warehouse::where('is_active', true)->get();
		foreach($lims_warehouse_list as $whouse)
		{
                ...
                $lims_product_warehouse_data->warehouse_id = $whouse->id;
...
//echo '<pre>'; print_r($lims_product_list); exit;
            return view('product.edit',compact('lims_product_list','types', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function importProduct(Request $request)
    {
        ...
        while($columns=fgetcsv($file))
        {
           ...
           $product->qty = 0;
           $product->alert_quantity = $data['quantity'];
...
/////////////////////////////////////////////////////////////////////////////////
        if($product->save())
        {
    		...
    		foreach($lims_warehouse_list as $whouse)
    		{
                    ...
                    $lims_product_warehouse_data->warehouse_id = $whouse->id;
                    $lims_product_warehouse_data->price = $product->price;
                    $lims_product_warehouse_data->qty = 0;
                	$lims_product_warehouse_data->save();
    		}
        }
