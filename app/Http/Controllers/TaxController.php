...
class TaxController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('tax')) {
            ...
            if($role->hasPermissionTo('tax_all')){
                ...
                $lims_tax_all = Tax::where('is_active', true)->where('id',auth()->user()->tax_id)->get();
            }
            // dd($role->hasPermissionTo('tax_add'));
            if(!$role->hasPermissionTo('tax_add')){

                $not_permitted = "Not permitted to add tax. Only admin has the rights!";
                $lims_taxes = Tax::where('is_active', true)->get();

                return view('tax.create', compact('lims_tax_all','not_permitted','lims_taxes'));
            }else{
    ...
    public function updateUserTax(Request $request){
        User::where('id',auth()->user()->id)->update(['tax_id'=>$request->tax_id]);
        return back()->with('message','Tax Updated Successfully');
    }
